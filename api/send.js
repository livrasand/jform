...
export default async function handler(req, res) {
    ...
    if (!owner) {
        return res
            .status(400)
            .json({ error: "Owner token is required" });
    }
    ...
    try {
        ...
        if (!record) {
            return res
                .status(403)
                .json({ error: "Invalid owner token" });
        }
        ...
        if (resendKey) {
                ...
                "https://api.resend.com/emails",
                {
                    ...
                    body: JSON.stringify({
                        from:
                            ...
                            "JForm <onboarding@resend.dev>",
                        to: email,
                        subject:
                            formTitle ||
                            "Nueva submission de JForm",
                        html: html,
                        attachments: req._jformAttachments?.length > 0 ? req._jformAttachments : undefined,
                    }),
                },
            );
            ...
            // Modo desarrollo - log
            console.log(
                "[JFORM RELAY] RESEND_API_KEY no configurado.",
                { to: email, data: formData },
            );
        }
        ...
        return res.status(200).json({
            status: "success",
            message:
                "Form data sent. The owner will receive it via email.",
        });
    } catch (err) {
...
function buildEmailHtmlPgp(title, encryptedBlock) {
    return (
        ...
        '<div style="padding:32px;background:linear-gradient(135deg,#5b8def,#4a7ad5);text-align:center">' +
        '<div style="display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:10px;background:rgba(255,255,255,0.2);font-size:1.2rem;font-weight:800;color:#fff;margin-bottom:8px">J</div>' +
        '<h1 style="margin:0;color:#fff;font-size:1.2rem;font-weight:600">' + esc(title) + " (PGP)" + "</h1>" +
        "</div>" +
        '<div style="padding:32px">' +
