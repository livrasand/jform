...
export default function FormView() {
  ...
  function applyTheme(theme) {
    ...
    if (theme.colors) {
      r.style.setProperty("--bg", theme.page_background || theme.colors.background_hex || "#fffafa");
      r.style.setProperty("--text", theme.colors.text_hex || theme.colors.text_hsl || "#1c1414");
      r.style.setProperty("--accent", theme.colors.primary_hex || "#4a7cf7");
      r.style.setProperty("--surface", theme.form_background || theme.colors.card_bg_hex || "#ffffff");
    } else {
      r.style.setProperty("--bg", theme.page_background || theme.background || "#fffafa");
      r.style.setProperty("--text", theme.text || "#1c1414");
      r.style.setProperty("--accent", theme.accent || "#4a7cf7");
      r.style.setProperty("--surface", theme.form_background || theme.card_bg || "#ffffff");
    }
  }
  ...
  async function loadForm() {
    ...
    const userRepo = `https://raw.githubusercontent.com/${encUser}/jform/main/forms/${encUser}/${encForm}`;
    const centralRepo = `https://raw.githubusercontent.com/livrasand/jform/main/forms/${encUser}/${encForm}`;
    ...
    try {
      ...
      setSchema(s);
      const hasEndpoint = s.endpoint || (s.transport && s.transport.type === "email");
      if (!hasEndpoint) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (schema.transport && schema.transport.type === "email") {
        ...
        r = await fetch("/api/send", { method: "POST", body: fd, signal: controller.signal });
      } else if (schema.transport && schema.transport.type === "webhook") {
        // Webhook directo
        r = await fetch(schema.transport.destination, { method: "POST", body: fd, signal: controller.signal });
      } else if (schema.endpoint) {
        // Schema v1 - endpoint directo
        r = await fetch(schema.endpoint, { method: "POST", body: fd, signal: controller.signal });
      } else {
      ...
      if (r.ok) {
        ...
        throw new Error(t("form_err_http") + r.status + t("form_err_suffix") + (errText ? ": " + errText : ""));
      }
    } catch (err) {
      if (err.name === "AbortError") {
        alert(t("form_err_prefix") + "La solicitud tardó demasiado. Verifica tu conexión o intenta de nuevo.");
      } else {
  ...
  if (state === "success") {
    ...
    const formBgSuccess = schema?.theme?.form_background || "var(--surface, #ffffff)";
    const formBorderSuccess = schema?.theme?.colors?.border_cmyk ? undefined : "1px solid var(--border)";
    return (
      <div
        className="form-wrap"
        style={{ textAlign: "center", padding: "48px 32px", background: formBgSuccess, border: formBorderSuccess }}
      >
  ...
  return (
    <>
    <div className="form-wrap" style={{ background: formBg, border: formBorder }}>
      ...
      <form id="f" encType="multipart/form-data" onSubmit={handleSubmit}>
        {(schema.elements || schema.fields).map((f) => {
          ...
          if (f.type === "section") {
            return (
              <div key={f.id} style={{ marginBottom: 8, marginTop: f.margin_top || 8 }}>
                {f.label && <h3 style={{ fontSize: "1rem", fontWeight: 700, margin: "0 0 4px" }}>{esc(f.label)}</h3>}
                {f.description && <p style={{ fontSize: "0.83rem", color: "var(--text-2)", margin: 0 }}>{esc(f.description)}</p>}
              </div>
            );
          }
          ...
          if (f.type === "image" || f.type === "video") {
            return (
              <div className="field-group" key={f.id}>
                ...
                {f.caption && <p style={{ fontSize: "0.85rem", color: "var(--text-2)", marginTop: 8 }}>{esc(f.caption)}</p>}
              </div>
            );
          }

          // Bordes por lado opcionales
          const borderStyle = (f.border_top || f.border_bottom || f.border_left || f.border_right) ? {
        ...
        <button type="submit" disabled={sending}>
          {sending ? t("form_sending") : (schema.settings?.submit_button_text || t("form_submit"))}
        </button>
      </form>
    </div>
    <div style={{
      ...
      <p style={{ margin: "0 0 6px" }}>
        {t("footer_disclaimer")}
        {" · "}
        <a href="/terms" style={{ color: "rgba(0,0,0,0.55)", textDecoration: "underline" }}>{t("footer_terms")}</a>
        {" · "}
        <a href="/privacy" style={{ color: "rgba(0,0,0,0.55)", textDecoration: "underline" }}>{t("footer_privacy")}</a>
      </p>
      <p style={{ margin: "0 0 16px" }}>
        {t("footer_suspicious")}{" "}
        <a href="https://github.com/livrasand/jform/issues" target="_blank" rel="noopener noreferrer" style={{ color: "rgba(0,0,0,0.55)", textDecoration: "underline" }}>{t("footer_report")}</a>
      </p>
      <img src="/jform-logo.svg" alt="JFORM" style={{ height: 32, opacity: 0.35, filter: "invert(1)", display: "block", margin: "0 auto" }} />
    </div>
    </>  
  );
}
    ...
    <div className="rating-group">
      {Array.from({ length: stars }, (_, i) => i + 1).map((v) => (
        <label
          ...
          style={{ color: v <= (hovered ?? selected ?? 0) ? "#f5a623" : "var(--border)" }}
      ...
      <div className="option-group">
        {(f.options || []).map((o, i) => (
          <label key={i} className="option-item">
            <input type="radio" name={name} value={attr(o.value)} required={f.required || false} />
            {esc(o.label)}
          </label>
        ))}
      </div>
              ...
              <tr key={row.id}>
                ...
                {(f.columns || []).map((col) => (
                  <td key={col.id}>
                    <input
                      type={inputType}
                      name={inputType === "radio" ? `${name}_${row.id}` : `${name}_${row.id}[]`}
                      value={col.id}
